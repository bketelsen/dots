...
in {
  home.packages =
    (with pkgs; [ arandr compton dunst i3lock nitrogen pasystray pavucontrol ]);
  ...
  programs = {
    urxvt = {
      enable = true;
      fonts = [ "xft:Cascadia Code:size=11" ];
    };
  };
    ...
    windowManager.i3 = {
      ...
      config = {
        ...
        keybindings = lib.mkOptionDefault {
          ...
          "${modifier}+b" =
            "exec --no-startup-id systemctl --user daemon-reload && systemctl --user restart pulseaudio";
          ...
          "XF86AudioRaiseVolume" = ''
            exec  --no-startup-id pactl set-sink-mute @DEFAULT_SINK@ false && pactl set-sink-volume @DEFAULT_SINK@ +5% && V=`pamixer --get-volume` && notify-send "Raising Volume to ''${V}%"'';

          "XF86AudioLowerVolume" = ''
            exec  --no-startup-id pactl set-sink-mute @DEFAULT_SINK@ false && pactl set-sink-volume @DEFAULT_SINK@ -5% && V=`pamixer --get-volume` && notify-send "Lowering Volume to ''${V}%"'';
